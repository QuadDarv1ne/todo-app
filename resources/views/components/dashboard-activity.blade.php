<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('activity-logs.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                Вся история →
            </a>
        </div>
        ...
        @if($recentLogs->isNotEmpty())
            <div class="space-y-3">
                @foreach($recentLogs as $log)
                    <div class="border-l-4 border-{{ $log->action_color }}-500 bg-{{ $log->action_color }}-50 p-3 rounded-r-lg">
                        <div class="flex items-start gap-2">
                            <span class="text-xl">{{ $log->action_icon }}</span>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-sm font-semibold text-gray-900">{{ $log->action_label }}</span>
                                    @if($log->model_type)
                                        <span class="text-xs px-2 py-0.5 bg-{{ $log->action_color }}-200 text-{{ $log->action_color }}-800 rounded-full">
