...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['title', 'description', 'icon', 'actionText' => null, 'actionUrl' => null, 'actionOnclick' => null]));
...
foreach (array_filter((['title', 'description', 'icon', 'actionText' => null, 'actionUrl' => null, 'actionOnclick' => null]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<div class="empty-state text-center py-16">
    ...
    <?php if($actionText): ?>
        <div class="mt-8">
            ...
            <?php elseif($actionOnclick): ?>
                <button onclick="<?php echo e($actionOnclick); ?>"
                        class="inline-flex items-center px-5 py-3 bg-indigo-600 border border-transparent rounded-lg font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                    <?php echo e($actionText); ?>
